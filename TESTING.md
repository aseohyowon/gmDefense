...
## 📱 태블릿 빌드 전 체크리스트 (현재 런타임 자동 구성 버전)

현재 프로젝트는 시작 시 `Menu` 씬, 게임 시작 버튼 클릭 시 `Game` 씬으로 이동한다.
`Game` 씬의 맵/적/유닛/UI는 런타임에서 자동 생성된다.

### 1) Build Settings 확인

1. Unity 메뉴: **File > Build Settings**
2. Scenes In Build 순서가 아래와 같은지 확인
     - `Assets/Menu.unity`
     - `Assets/Game.unity`
3. Android 플랫폼으로 Switch Platform 되어 있는지 확인

### 2) 컴파일 에러 0개 확인

1. Unity Console에서 빨간 에러가 0개인지 확인
2. 에러가 있으면 먼저 해결 후 빌드 진행

### 3) 빠른 플레이 점검 (Editor Play 30초)

1. Play 실행 → 메뉴 화면 표시 확인
2. 게임 시작 버튼 클릭 → 게임 씬 전환 확인
3. 상단 정보(물결/적 수), 하단 패널(유닛/골드/소환) 표시 확인
4. 적이 길을 따라 이동하는지 확인
5. 소환 버튼 클릭 시 골드가 줄고 유닛 수가 증가하는지 확인

### 4) Android 빌드 권장 옵션

- Development Build: ON (문제 확인용)
- Script Debugging: OFF (성능 유지)
- Run Device: 연결된 태블릿 선택

### 5) 태블릿에서 확인할 포인트

- 첫 화면이 메뉴인지
- 게임 시작 후 맵/길/적/유닛이 모두 보이는지
- 프레임 드랍 없이 웨이브가 진행되는지
- 소환 버튼 비활성/활성 상태가 골드에 맞게 변하는지

### 6) 자주 겪는 문제 빠른 해결

- **이전 화면이 그대로 보임**
    - 기존 앱 삭제 후 재설치
    - Build Settings 씬 순서 재확인

- **적 카운트만 있고 적이 안 보임**
    - Console 에러 확인
    - Editor Play에서 먼저 같은 증상 재현 여부 확인

- **빌드 실패(스크립트 컴파일 오류)**
    - Console 첫 번째 에러부터 해결
    - 에러 0개 만든 뒤 다시 Build
